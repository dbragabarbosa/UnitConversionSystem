...
def test_polegadas_para_jardas():
    assert round(converter.polegadas_para_jardas(36), 6) == 1

def test_jardas_para_polegadas():
    assert converter.jardas_para_polegadas(1) == 36
...
def test_milhas_para_polegadas():
    assert round(converter.milhas_para_polegadas(1), 2) == 63360

def test_polegadas_para_milhas():
    assert round(converter.polegadas_para_milhas(63360), 6) == 1
...
def test_milhas_para_jardas():
    assert converter.milhas_para_jardas(1) == 1760

def test_jardas_para_milhas():
    assert converter.jardas_para_milhas(1760) == 1
